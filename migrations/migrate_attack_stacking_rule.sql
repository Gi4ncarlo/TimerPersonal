...
BEGIN
  ...
  -- Prevent stacking negative states (attacks like sabotaje/parasites) on a target from ANY user
  IF v_power_type IN ('sabotaje', 'parasito_agresivo', 'parasito_lento') AND EXISTS (
      SELECT 1 FROM user_active_powers 
      WHERE target_user_id = p_target_user_id 
      AND power_type IN ('sabotaje', 'parasito_agresivo', 'parasito_lento') 
      AND expires_at > now()
  ) THEN
    RETURN jsonb_build_object('success', false, 'error', 'TARGET_ALREADY_UNDER_ATTACK');
  END IF;
